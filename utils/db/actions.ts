...
import { eq, sql, and, desc, ne, sum } from "drizzle-orm";
...
export async function getLeaderboard() {
  try {
    // Fetch total points for each user and display in leaderboard
    const leaderboard = await db
      .select({
        userId: Transactions.userId,
        userName: Users.name,
        totalPoints: sum(Transactions.amount), // Summing the amount for each user
      })
      .from(Transactions)
      .leftJoin(Users, eq(Transactions.userId, Users.id)) // Join Transactions with Users
      .groupBy(Transactions.userId, Users.name) // Group by userId and userName to aggregate points
      .orderBy(desc(sum(Transactions.amount))) // Order by total points in descending order
      .execute();
...
//         userId: Rewards.userId,
//         userName: Users.name,
//         totalPoints: sum(Rewards.points), // Summing the points for each user
//       })
//       .from(Rewards)
//       .leftJoin(Users, eq(Rewards.userId, Users.id)) // Join Rewards with Users
//       .groupBy(Rewards.userId, Users.name) // Group by userId and userName to aggregate points
//       .orderBy(desc(sum(Rewards.points))) // Order by total points in descending order
//       .execute();
